...
% amplitude has dinamics that allow the demodulation dither to converge to
% zero near the neighborhood of the resonant frequency.
...
Tfin = 300;         % Final simulation time
...
l_rms = 100;   % Number of samples for the cumulative averaging filter
x_rms_0 = 0.00; % Initial RMS filter state

% Discrete-time extremum seeking control hyperparameters
K_g = 500;          % Measurement gain
omega_l = 10;       % Low-pass filter cutoff frequency in rad/s
omega_h = 3;        % High-pass filter cutoff frequency in rad/s
K_esc = 2.5;        % Modulation gain
omega_esc = 4;      % Dither signal frequency in rad/s
A_esc_d = 1.2;      % Demodulation dither signal initial amplitude

% ESC modulation dither signal amplitude hyperparameters
A_esc_m_0 = 1.2;    % Initial amplitude
delta_m = 0.002;    % Gradient norm bound for amplitude dynamics behavior switching
alpha_m = 0.1;      % Gradient descent rate factor away from resonant frequency neighborhood
lambda_m = 0.025;   % Amplitude vanishing rate factor near resonant frequency neighborhood

t_en = 2*(l_rms + 1)*Ts;        % Time at which ESC is enabled
k_en = floor(t_en/Ts_esc);      % ESC step at which ESC is enabled

%% Running the simulation in Simulink

out = sim('msd_resonant_frequency_seeking_DT_vanishing_dither.slx');
