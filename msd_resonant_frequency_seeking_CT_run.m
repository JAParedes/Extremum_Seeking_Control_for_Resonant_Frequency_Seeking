...
% continuous-time. This simulation example is applied to the
...
Tfin = 350; % Final simulation time.
...
% Continuous-time nonlinear oscillator parameters
...
T_d = 2;    % Time delay for moving RMS calculation.

% Continuous-time extremum seeking control hyperparameters
K_g = 500;          % Measurement gain
omega_l = 4;       % Low-pass filter cutoff frequency in rad/s
omega_h = 2;        % High-pass filter cutoff frequency in rad/s
K_esc = 2.5;          % Modulation gain
omega_esc = 1;    % Dither signal frequency in rad/s
A_esc_d = 1;      % Demodulation dither signal initial amplitude
A_esc_m = 1;      % Modulation dither signal amplitude

t_stop = 300;       % Time after which the modulation dither becomes zero

%% Running the simulation in Simulink

out = sim('msd_resonant_frequency_seeking_CT.slx');
